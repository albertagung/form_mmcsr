...
for (let i = 1; i <= 31; i++) {
  ...
  $('#dayAdmission').append(`<option value="${i}" selected>${i}</option>`)
}
...
for (let month = 0; month < months.length; month++) {
  ...
  $('#monthAdmission').append(`<option value="${month+1}" selected>${months[month]}</option>`)
}
...
for (let year = minYear; year <= maxYear; year++) {
  ...
  $('#yearAdmission').append(`<option value="${year}" selected>${year}</option>`)
}